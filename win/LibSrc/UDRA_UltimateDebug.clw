...
ANSI_NAME EQUATE('')
  ...
  MAP
    ...
    MODULE('C%V%RUN%X%')
      ...
      Cla_FieldName(SIGNED FEQ),*CSTRING,RAW,NAME('Cla$FIELDNAME')
    END
...
szMSG         &CSTRING
  ...
  ! RA.2019.01.08.TUE: Had a conversation with Mark Goldberb about the Duplicates code
...
!==============================================================================
  CODE
  SELF.sCount                   &= NULL                     ! RA.2025.09.24 - Count String
  ...
  !SELF.LevelWarn                = 20
  SELF.LevelWarn                = 200                       ! RA.2019.04.07.SUN: Changed: 20 -> 200 (20 a bit too low)
  SELF.osMode                   = FALSE
  SELF.Duplicates               = 200                       ! (50->200) MG Policy: only track lastmsg,NumberSame if going to do something with them....
  SELF.NumberSame               = 0
  SELF.ThisMsg                  = ''
  SELF.LastMsg                  = ''
...
!==============================================================================
  CODE
  DISPOSE(SELF.sCount)                                      ! RA.2025.09.24 - Release
  SELF.sCount                   &= NULL                     ! RA.2025.09.24 - Count String
...
!==============================================================================
  CODE
  RETURN '[T' & FORMAT(SELF.lThread,@N02) & ']' & pMSG
...
!==============================================================================
  CODE
  RETURN SELF.LOW_THREAD('[L' & FORMAT(SELF.gLevels,@N03) & ']' & SELF.LOW_COUNT() & pMSG)
...
!==============================================================================
ThreadDebugCore.LOW_COUNT                   PROCEDURE()!,STRING                 ! RA.2025.09.24.WED: Added
!==============================================================================
! RA.2025.09.24.WED: Visual to nesting GLOBAL level ThreadDebugCore.gLevels
!   You initialize the first SELF.sCount on the SELF.Construct.
!   You destroy the last SELF.scCount on the SELF.Destruct.
!   When you come in to this procedure you dispose the old SELF.sCount and 
!   create the new SELF.sCount with the correct length.
!   You fill the string with the correct visual pattern and then pass
!   the correct string back.
!==============================================================================
I       LONG
  CODE
  DISPOSE(SELF.sCount)                                                          ! RA.2025.09.24.WED
 !SELF.sCount &= NEW STRING(SELF.gLevels)
 !SELF.sCount = ALL('-',SELF.gLevels)
  SELF.sCount &= NEW STRING(SELF.gLevels * 2 + 1)
  SELF.sCount = ''
  LOOP I = 1 TO SELF.gLevels
    SELF.sCount = CLIP(SELF.sCount) & ' -'
  END
  SELF.sCount = CLIP(SELF.sCount) & ' '
  RETURN (CHOOSE(SELF.gLevels>0,SELF.sCount,''))
...
!==============================================================================
  ...
  RETURN
!==============================================================================
ThreadDebugCore.EnterDo                     PROCEDURE(STRING pMSG)              ! RA.2025.09.21.SUN: Added
!==============================================================================
  CODE
  SELF.gLevels        += 1
  SELF.Debug(':ENTERDO ' & CLIP(pMSG) & '')
  RETURN
!==============================================================================
ThreadDebugCore.LevelDo                     PROCEDURE(STRING pMSG)              ! RA.2025.09.21.SUN: Added
!==============================================================================
  CODE
  SELF.Debug(' ' & CLIP(pMSG) & '')
  RETURN
!==============================================================================
ThreadDebugCore.ExitsDo                     PROCEDURE(STRING pMSG)              ! RA.2025.09.21.SUN: Added
!==============================================================================
  CODE
  SELF.Debug(':EXITSDO ' & CLIP(pMSG) & '')
  SELF.gLevels        -= 1
  RETURN
!==============================================================================
!!! <summary>
...
NamePipe   SHORT                                                                !RAYS 
  ...
  LOOP
    ...
    IF ISGROUP(xaGroup,_Where)
      ...
      _Field   = WHO(xaGroup,_Where)                                            !RAYS
	  NamePipe = INSTRING('|', _Field)                                          !RAYS
	  IF NamePipe THEN _Field[NamePipe] = '<0>' END                             !RAYS
      SELF.Debug(xLinePrefix & ' ' & _Field & '')                               !RAYS
      _Where += 1
      CYCLE
    END
    ...
    CASE _Type
    OF TypeUFO:DATE   ;  _Data = CLIP(FORMAT(_What,@D010-B))
    OF TypeUFO:TIME   ;  _Data = CLIP(FORMAT(_What,@T04B))
    OF TypeUFO:STRING ;  _Data = CLIP       (_What)
    END
    ! RA.2019.01.07: Output a better and improved message.
	...
	NamePipe = INSTRING('|', _Field)                                            !RAYS
	IF NamePipe THEN _Field[NamePipe] = '<0>' END                               !RAYS
    SELF.Debug(xLinePrefix & ' ' & _Field & ' ' & SELF.DescribeDataTypeShort(_Type,_Size) & ' = [ <39>' & _Data & '<39> ]')
    _Where += 1
  END
  RETURN
!==============================================================================
...
UFO                             &TUFO_CallInterface
  ...
  !END-OMIT('***',_C70_)
!==============================================================================
ThreadDebugCore.DescribeDataType            PROCEDURE(LONG xType) !,STRING  !<-- assumes a TypeUFO:*  (see equates.clw)
!==============================================================================
RetVal CSTRING(25) !TypeUFO:PDECIMAL has a length of 17
  CODE
  CASE xType                                                    ! ITEMIZE(1),PRE(TypeUFO)
    OF TypeUFO:BYTE        ; RetVal = 'TypeUFO:BYTE'            ! EQUATE
    OF TypeUFO:SHORT       ; RetVal = 'TypeUFO:SHORT'           ! EQUATE
    OF TypeUFO:USHORT      ; RetVal = 'TypeUFO:USHORT'          ! EQUATE
    OF TypeUFO:DATE        ; RetVal = 'TypeUFO:DATE'            ! EQUATE
    OF TypeUFO:TIME        ; RetVal = 'TypeUFO:TIME'            ! EQUATE
    OF TypeUFO:LONG        ; RetVal = 'TypeUFO:LONG'            ! EQUATE
    OF TypeUFO:ULONG       ; RetVal = 'TypeUFO:ULONG'           ! EQUATE
    OF TypeUFO:SREAL       ; RetVal = 'TypeUFO:SREAL'           ! EQUATE
    OF TypeUFO:REAL        ; RetVal = 'TypeUFO:REAL'            ! EQUATE
    OF TypeUFO:DECIMAL     ; RetVal = 'TypeUFO:DECIMAL'         ! EQUATE
    OF TypeUFO:PDECIMAL    ; RetVal = 'TypeUFO:PDECIMAL'        ! EQUATE
    OF TypeUFO:BFLOAT4     ; RetVal = 'TypeUFO:BFLOAT4'         ! EQUATE(13)
    OF TypeUFO:BFLOAT8     ; RetVal = 'TypeUFO:BFLOAT8'         ! EQUATE
    OF TypeUFO:STRING      ; RetVal = 'TypeUFO:STRING'          ! EQUATE(18)
    OF TypeUFO:CSTRING     ; RetVal = 'TypeUFO:CSTRING'         ! EQUATE
    OF TypeUFO:PSTRING     ; RetVal = 'TypeUFO:PSTRING'         ! EQUATE
    OF TypeUFO:MEMO        ; RetVal = 'TypeUFO:MEMO'            ! EQUATE
    OF TypeUFO:BLOB        ; RetVal = 'TypeUFO:BLOB'            ! EQUATE(27)
!
    OF TypeUFO:UFO         ; RetVal = 'TypeUFO:UFO'             ! EQUATE(15)                  ! RA.2020.03.08: Added
    OF TypeUFO:PICTURE     ; RetVal = 'TypeUFO:PICTURE'         ! EQUATE(29)                  ! RA.2020.03.08: Added
    OF TypeUFO:REFERENCE   ; RetVal = 'TypeUFO:REFERENCE'       ! EQUATE(31)                  ! RA.2020.03.08: Added
    OF TypeUFO:FILEREF     ; RetVal = 'TypeUFO:FILEREF'         ! EQUATE(32)                  ! RA.2020.03.08: Added
    OF TypeUFO:KEYREF      ; RetVal = 'TypeUFO:KEYREF'          ! EQUATE(33)                  ! RA.2020.03.08: Added
    OF TypeUFO:QUEUEREF    ; RetVal = 'TypeUFO:QUEUEREF'        ! EQUATE(34)                  ! RA.2020.03.08: Added
    OF TypeUFO:CLASSREF    ; RetVal = 'TypeUFO:CLASSREF'        ! EQUATE(35)                  ! RA.2020.03.08: Added
    OF TypeUFO:WINDOWREF   ; RetVal = 'TypeUFO:WINDOWREF'       ! EQUATE(36)                  ! RA.2020.03.08: Added
    OF TypeUFO:VIEWREF     ; RetVal = 'TypeUFO:VIEWREF'         ! EQUATE(37)                  ! RA.2020.03.08: Added
    OF TypeUFO:BLOBREF     ; RetVal = 'TypeUFO:BLOBREF'         ! EQUATE(38)                  ! RA.2020.03.08: Added
    OF TypeUFO:NAMEREF     ; RetVal = 'TypeUFO:NAMEREF'         ! EQUATE(39)                  ! RA.2020.03.08: Added
    OF TypeUFO:LIKE        ; RetVal = 'TypeUFO:LIKE'            ! EQUATE(40)                  ! RA.2020.03.08: Added
    OF TypeUFO:TYPE        ; RetVal = 'TypeUFO:TYPE'            ! EQUATE(41)                  ! RA.2020.03.08: Added
    OF TypeUFO:BSTRING     ; RetVal = 'TypeUFO:BSTRING'         ! EQUATE(42)                  ! RA.2020.03.08: Added
    OF TypeUFO:ASTRING     ; RetVal = 'TypeUFO:ASTRING'         ! EQUATE(43)                  ! RA.2020.03.08: Added
!
  ELSE                      ; RetVal = 'TypeUFO:Unknown['& xType &']'
                           !31 seems to be returned for references (and ANY)  (c6.9056)
  END
  RETURN RetVal
!==============================================================================
! RA.2018.12.30.SUN: Give me a DataTyep(Lenght) without a prefix. 
ThreadDebugCore.DescribeDataTypeShort       PROCEDURE(LONG xType, LONG xSize) !,STRING  !<-- assumes a TypeUFO:*  (see equates.clw)
!==============================================================================
RetVal  CSTRING(25) !TypeUFO:PDECIMAL has a length of 17
iDo     LONG(0)
  CODE
 ...
 !RetVal = SUB(RetVal, 10, LEN(RetVal) - 9) ! SIZE('DataType:') = 9
  RetVal = SUB(RetVal,  9, LEN(RetVal) - 8) ! SIZE('TypeUFO:')  = 8
  CASE xType
    OF TypeUFO:STRING
  OROF TypeUFO:CSTRING
  OROF TypeUFO:PSTRING
  OROF TypeUFO:MEMO
  OROF TypeUFO:BLOB
    RetVal = RetVal & '(' & xSize & ')'
  END
...
NamePipe	SHORT,AUTO                                                          !RAYS
  ...
  LOOP X = 10000 TO 1 BY -1
    ...
    END
  END
  ! RA.2022.09.10: Address me 
  CLEAR(FieldQ)
  FieldQ.Name   = '*THIS*'
  FieldQ.Type   = 'ADDRESS'
  FieldQ.Value  = CLIP(ADDRESS(pGroup))
  ADD(FieldQ)
  !
  LOOP X = 1 TO M
    CLEAR(FieldQ)
    IF NumFields >= X
	  ...
	  NamePipe      = INSTRING('|', FieldQ.Name) 								!RAYS
	  IF NamePipe THEN FieldQ.Name[NamePipe] = '<0>' END 						!RAYS
      ...
      IF NOT ISGROUP(pGroup, X)
        ...
        OF TypeUFO:DATE         ;  _Data = CLIP(FORMAT(_What,@D010-B))
        OF TypeUFO:TIME         ;  _Data = CLIP(FORMAT(_What,@T04B))
        !OF TypeUFO:REFERENCE    ;  _Data = CLIP(LEFT(FORMAT(_What,@N_10B)))
        !OF TypeUFO:REFERENCE    ;  _Data = CLIP(ADDRESS(_What))
        OF TypeUFO:STRING       ;  _Data = CLIP       (_What)
        END
        FieldQ.Value            = _Data
      END
...
StripPrefixLength   				BYTE,AUTO
                  ***---***
FieldQ                              QUEUE
Header                                  CSTRING(256)    ! Field name <13,10>TYPE(Size)
...
!                     LIST, AT(4,364,668,48), USE(?MessageList), VSCROLL, FROM(MsgLineQ)
!                   END
!Window    WINDOW('Dump Queue'),SYSTEM,AT(,,676,316),CENTER,FONT('Segoe UI',10),GRAY,DOUBLE
Window    WINDOW('Dump Queue'),SYSTEM,AT(,,676,216),CENTER,FONT('Segoe UI',10),GRAY,RESIZE
...
ColumnNo        SHORT(0)
  ...
  LOOP FieldNo = 1 TO NumFields
    ...
    ?DebugList{PROPLIST:Header                        , ColumnNo} = UPPER(FieldQ.Header) & '<13><10>' & SELF.DescribeDataTypeShort(FieldQ.Type,FieldQ.Size)
    ...
    CASE FieldQ.Type
    OF TypeUFO:DATE        ; ?DebugList{PROPLIST:Picture  , ColumnNo} = '@D010-B'
    OF TypeUFO:TIME        ; ?DebugList{PROPLIST:Picture  , ColumnNo} = '@T04B'
    !OF TypeUFO:REFERENCE   ; ?DebugList{PROPLIST:Picture  , ColumnNo} = '@N_10B'
...
!   ?DebugList{PROPLIST:HeaderCenter                  , ColumnNo} = True
    ...
    CASE FieldQ.Type
      OF TypeUFO:DATE
    OROF TypeUFO:TIME
	OROF TypeUFO:REFERENCE
      ?DebugList{PROPLIST:Left                        , ColumnNo} = True
      ?DebugList{PROPLIST:LeftOffset                  , ColumnNo} = 1
    END
      ?DebugList{PROPLIST:Left                        , ColumnNo} = True        ! RA.2022.03.19: Everything Left
      ?DebugList{PROPLIST:LeftOffset                  , ColumnNo} = 1           ! RA.2022.03.19: Everything Left
...
!
  ...
  !SELF.Debug('ST::DebugQueue/LoadFieldQ: RECORDS(pQueue)='& RECORDS(pQueue))
    IF RECORDS(pQueue) > 0 AND pNoTouch <> 1
      ...
      LOOP RecNo = 1 TO RECORDS(pQueue)
        GET(pQueue, RecNo)
	!
	IF (FieldQ.Width < LEN(FieldQ.Header))                                         THEN FieldQ.Width = LEN(FieldQ.Header)                                          END
	IF (FieldQ.Width < LEN(SELF.DescribeDataTypeShort(FieldQ.Type,FieldQ.Size)))   THEN FieldQ.Width = LEN(SELF.DescribeDataTypeShort(FieldQ.Type,FieldQ.Size))    END
	!
  !SELF.Debug('ST::DebugQueue/LoadFieldQ: RecNo='& RecNo &'; '& SELF.DescribeDataTypeShort(FieldQ.Type,FieldQ.Size) &'; _What='& _What)
        IF FieldRef <> ''
          ...
          CASE(FieldQ.Type)
          OF TypeUFO:DATE          ; DataLength = 8
          OF TypeUFO:TIME          ; DataLength = 8
          OF TypeUFO:REFERENCE     ; DataLength = 8
          END
...
DataWidth       SHORT,AUTO
  ...
  HeaderWidth  = HeaderLength * WidthMultiplier:Wide
...
!==============================================================================
ThreadDebugCore.StringToHex                 PROCEDURE(*STRING binData) !,STRING
!==============================================================================
hex             &string
i               long, auto
j               long, auto
  CODE
  IF LEN(binData) = 0 THEN
    RETURN hex
  END
    
  hex &= NEW STRING(LEN(binData) * 2)         ! Need two characters for each byte 
  j = 1
  LOOP i = 1 TO LEN(binData)
    hex[j : j + 1] = SELF.ByteToHex(VAL(binData[i]))
    j += 2
  END
  RETURN hex
!==============================================================================
ThreadDebugCore.ByteToHex 					PROCEDURE(BYTE xByte) !,STRING
!==============================================================================
hex 		STRING(2), auto
hexChars    STRING('0123456789abcdef')
  CODE
  hex[1] = hexChars[bshift(xByte, -4) + 1]
  hex[2] = hexChars[band(xByte, 0fh) + 1]
  RETURN hex
...
!!	OMIT('!+++!!!+++!')
	...
	END
			...
			END
		ELSE
			FieldQ.Name   = WHO(pGroup2, X)                                     !RAYS
	        NamePipe = INSTRING('|', FieldQ.Name) 							    !RAYS
	        IF NamePipe THEN FieldQ.Name[NamePipe] = '<0>' END 				    !RAYS
			DO AssignValue2
		END
