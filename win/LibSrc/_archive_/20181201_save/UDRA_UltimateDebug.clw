...
UltimateDebugCore.Construct                PROCEDURE
   ...
   SELF.DebugMe         = FALSE
   IF (SELF.DebugMe)
     SELF.ODS_THREAD(':DEBUGME UltimateDebugCore.Construct')
     SELF.ODS_THREAD(':DEBUGME:FLAG UltimateDebugCore.Construct SELF.DebugMe=(' & SELF.DebugMe & ')')
   END   
   RETURN
!==============================================================================
UltimateDebugCore.Destruct                 PROCEDURE
   ...
   IF (SELF.DebugMe)
     !SELF.ODS_THREAD(':DEBUGME:FLAG UltimateDebugCore.Destruct SELF.DebugMe=(' & SELF.DebugMe & ')')  
     SELF.ODS_THREAD(':DEBUGME UltimateDebugCore.Destruct')
   END
   RETURN
!==============================================================================
...
!==============================================================================
!==============================================================================
UltimateDebugCore.ODS_RYB                  PROCEDURE(STRING pDebugString)
   CODE
   ODS_RYB(CLIP(pDebugString))
   RETURN
!==============================================================================
!==============================================================================
UltimateDebugCore.ODS_PREFIX               PROCEDURE(STRING pDebugString)
   CODE
   SELF.RawODS(SELF.DebugPrefix & CLIP(pDebugString))
   RETURN
!==============================================================================
UltimateDebugCore.ODS_THREAD               PROCEDURE(STRING pDebugString)
   CODE
   SELF.ODS_PREFIX('[T' & SELF.lThread & ']' & CLIP(pDebugString))
   RETURN
!==============================================================================
UltimateDebugCore.ODS_ENTER                PROCEDURE(STRING pDebugString)
   CODE
   IF (SELF.DebugMe) THEN  
     SELF.ODS_THREAD(':DEBUGME UltimateDebugCore.ODS_ENTER')
   END
   SELF.gLevels        += 1
   SELF.ODS_THREAD('[L' & FORMAT(SELF.gLevels,@N03) & ']' & CLIP(pDebugString))
   RETURN
!==============================================================================
UltimateDebugCore.ODS_LEVEL                PROCEDURE(STRING pDebugString)
   CODE
   IF (SELF.DebugMe) THEN  
     SELF.ODS_THREAD(':DEBUGME UltimateDebugCore.ODS_LEVEL')
   END
   SELF.ODS_THREAD('[L' & FORMAT(SELF.gLevels,@N03) & ']' & CLIP(pDebugString))
   RETURN
!==============================================================================
UltimateDebugCore.ODS_EXITS                PROCEDURE(STRING pDebugString)
   CODE
   IF (SELF.DebugMe) THEN  
     SELF.ODS_THREAD(':DEBUGME UltimateDebugCore.ODS_EXITS')
   END
   SELF.ODS_THREAD('[L' & FORMAT(SELF.gLevels,@N03) & ']' & CLIP(pDebugString))
   SELF.gLevels        -= 1
   RETURN
...
UltimateDebugCore.Enter                    PROCEDURE(STRING psEXE, STRING psMOD, STRING psPRC)
   CODE
   !SELF.DebugSELF('UltimateDebugCore.Enter(...)')
   !SELF.szEXE           = CLIP(psEXE) & '<0>' 
   !SELF.szMOD           = CLIP(psMod) & '<0>'
   !SELF.szPRC           = CLIP(psPRC) & '<0>'
   SELF.ODS_ENTER(':ENTER (' & CLIP(psEXE) & '=' & CLIP(psMOD) & '=' & CLIP(psPRC) & ')')
   RETURN
!==============================================================================
UltimateDebugCore.Enter                    PROCEDURE(STRING psMSG)
   CODE
   !SELF.DebugSELF('UltimateDebugCore.Enter(.)')
   SELF.ODS_ENTER(':ENTER ' & CLIP(psMSG) & '')
   RETURN
!==============================================================================
UltimateDebugCore.Level                    PROCEDURE(STRING psMSG)
   CODE
   !SELF.DebugSELF('UltimateDebugCore.Level')
   SELF.ODS_LEVEL(' ' & CLIP(psMSG) & '')
   RETURN
!==============================================================================
UltimateDebugCore.Exits                    PROCEDURE(STRING psMSG)
   CODE
   !SELF.DebugSELF('UltimateDebugCore.Exits(.)')
   SELF.ODS_EXITS(':EXITS ' & CLIP(psMSG) & '')
   RETURN
!==============================================================================
UltimateDebugCore.Exits                    PROCEDURE(STRING psEXE, STRING psMOD, STRING psPRC)
   CODE
   !SELF.DebugSELF('UltimateDebugCore.Exits(...)')
   SELF.ODS_EXITS(':EXITS (' & CLIP(psEXE) & '=' & CLIP(psMOD) & '=' & CLIP(psPRC) & ')')
   !SELF.szEXE           = '<0>'
   !SELF.szMOD           = '<0>'
   !SELF.szPRC           = '<0>'
   RETURN
...
Hi                                      LONG
  ...
  PUTINI('Debuger','EventOffset',SELF.EventOffset)
!==============================================================================
UltimateDebugCore.Debug                    PROCEDURE(STRING pDebugString)
  CODE
  !SELF.ODS_THREAD(':DEBUG UltimateDebugCore.Debug(...)')
  SELF.ODS_THREAD(' ' & CLIP(pDebugString) & '')
!  IF ~SELF.DebugOff
!      SELF.DebugToODS(pDebugString)
!  END
!  IF  SELF.SaveToFile
!      SELF.DebugToFile(pDebugString)
!  END
  RETURN
!==============================================================================
UltimateDebugCore.DebugOut                 PROCEDURE(STRING pDebugString)
  ...
  RETURN
!==============================================================================
UltimateDebugCore.DebugEntry               PROCEDURE(STRING pDebugString)
!==============================================================================
! RA.2014.03.29 - Make the nesting level greater
    CODE
!    SELF.NestedLevel += 1
	SELF.Enter('===ENTRY=== ' & CLIP(pDebugString))
    RETURN
!==============================================================================
UltimateDebugCore.DebugLevel               PROCEDURE(STRING pDebugString)
!==============================================================================
! RA.2014.03.29 - Show me the current level
    CODE
    SELF.Level(CLIP(pDebugString))
    RETURN
!==============================================================================
UltimateDebugCore.DebugExit                PROCEDURE(STRING pDebugString)
!==============================================================================
! RA.2014.03.29 - Make the nesting level lesser
    CODE
!    SELF.NestedLevel -= 1
	SELF.Exits('===EXIT=== ' & CLIP(pDebugString))
    RETURN
!==============================================================================
UltimateDebugCore.DebugToODS               PROCEDURE(STRING pDebugString)
  CODE
  IF SELF.DebugNoCR OR INSTRING('<13>',pDebugString,1,1)=0
     SELF.ODS_RYB(pDebugString)
  ELSE
     DO Debug:SplitByCR
  END
...
LineNum      LONG,AUTO
  ...
  UString.Split('<13><10>')                     ! RA.2014.03.30 - CR,LF = Windows
  LOOP LineNum = 1 TO UString.Records()
    SELF.ODS_RYB(UString.GetLine(LineNum))
  END
!==============================================================================
UltimateDebugCore.DebugToFile_StartEachRow         PROCEDURE()
  CODE
  RETURN '[' & FORMAT(TODAY(),@D17) & '-' & FORMAT(CLOCK(),@T7) & ']- '
!==============================================================================
...
STRING                  STRING(512)
  ...
  IF SELF.DebugNoCR OR INSTRING('<13>',pDebugString,1,1)=0
     ASC1:String = SELF.DebugToFile_StartEachRow() & CLIP(pDebugString)
     ADD(ASC1:ASCIIFile)
  ELSE
     DO Debug:SplitByCR
  END
...
LineNum      LONG,AUTO
  CODE
  UString.Assign(pDebugString)
  UString.Split('<13,10>')                      ! RA.2014.04.03 - CR,LF = Windows
  LOOP LineNum = 1 TO UString.Records()
    ASC1:String = SELF.DebugToFile_StartEachRow() & UString.GetLine(LineNum)
    ADD(ASC1:ASCIIFile)
  END
...
!       OR b) you are filtering the message
  CODE
  SELF.ODS_RYB('DBGVIEWCLEAR') !Written this way to get the prefix, so the message won't be filtered
...
!==============================================================================
  ...
  SELF.UD.ODS_RYB('UltimateDebugTraceFile.Init(' & self.myfilename & ')')
...
!==============================================================================
  CODE
  SELF.UD.ODS_RYB('UltimateDebugTraceFile.Kill()')
...
!==============================================================================
  CODE
  !SELF.UD.ODS_RYB('UltimateDebugTraceFile.FileCallBackInterface.FunctionCalled(' & opCode & ')')
...
!==============================================================================
  CODE
  !SELF.UD.ODS_RYB('UltimateDebugTraceFile.FileCallBackInterface.FunctionDone(' & opCode & ')')
...
? OF DriverOp:RestoreState              ; SELF.FileAction = 'RESTORESTATE'
  ...
  END
  !SELF.UD.ODS_RYB('UltimateDebugTraceFile.GetLastAction(' & opCode & ' = ' & SELF.FileAction & ')')
  RETURN
!==============================================================================
...
ThreadDebugLocal.Construct               PROCEDURE
  CODE
  IF (SELF.DebugMe)
    PARENT.ODS_THREAD(':DEBUGME ThreadDebugLocal.Construct')
  END
  RETURN
!------------------------------------------------------------------------------
ThreadDebugLocal.Destruct                PROCEDURE
  CODE
  IF (SELF.DebugMe)
    PARENT.ODS_THREAD(':DEBUGME ThreadDebugLocal.Destruct')
  END
  RETURN
!------------------------------------------------------------------------------
